...
/datum/game_mode/proc/equip_traitor(mob/living/carbon/human/traitor_mob, safety = 0)
	...
	var/loc = ""
	var/obj/item/R = locate(/obj/item/clothing/gloves/pda) in traitor_mob.contents //Hide the uplink in a PDA if available, otherwise radio
	if(!R)
	...
	if (!R)
		...
		if (istype(R, /obj/item/device/radio))
		...
		else if (istype(R, /obj/item/clothing/gloves/pda))
			...
			R.hidden_uplink = T
			T.uplink_owner = "[traitor_mob.key]"
			var/obj/item/clothing/gloves/pda/P = R
