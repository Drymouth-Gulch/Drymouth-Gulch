	...
	if(!activating && !activated && !teleporting)
		...
		if(user && ishuman(user))
			if(user.wear_suit == src)
				user << "\nChronosuit login: root"
				user << "Password:\n"
				user << "root@Chronosuit# timemachine --start\n"
				if(user.head && istype(user.head, /obj/item/clothing/head/helmet/space/chronos))
					...
					user << "\[ <span style='color: #00ff00;'>ok</span> \] Initializing timemachine-view"
					new_camera(user)
...
/obj/item/clothing/suit/space/chronos/proc/deactivate(force = 0, silent = 0)
	if(activated && (!teleporting || force))
		...
		if(user && ishuman(user))
			if(user.wear_suit == src)
				if(!silent)
					user << "\nroot@ChronosuitMK4# timemachine --stop\n"
				if(camera)
					if(!silent)
						user << "\[ <span style='color: #ff5500;'>ok</span> \] Sending TERM signal to timemachine-view" //yes I know they aren't a different color when shutting down, but they were too similar at a glance
					qdel(camera)
				if(helmet)
					if(!silent)
